...
public class MF_fastALS extends TopKRecommender {
  ...
  boolean showLoss;
	...
	public double[] precs_vec = new double[maxIter];
        public double[] grad_vec = new double[maxIter];
  ...
  public double w_new = 1;
	...
	public void setTrain(SparseMatrix trainMatrix) {
		...
		for (int u = 0; u < userCount; u ++)
			for (int i : this.trainMatrix.getRowRef(u).indexList())
				W.setValue(u, i, trainMatrix.getValue(u,i));
	}
	...
	public void buildModel() throws IOException{		
                ...
                System.out.printf("Run for FastALS: showProgress=%s, factors=%d, maxIter=%d, reg=%f, w0=%.2f",
				showProgress, factors, maxIter, reg, w0);
		System.out.println("====================================================");
		double loss_pre = Double.MAX_VALUE;
                double[] res = new double[3];
                PrintWriter writer = new PrintWriter (new FileOutputStream("MF_fastALS.progress"));
		for (int iter = 0; iter < maxIter; iter ++) {
			...
			if (showProgress)
				...
				res = showProgress(iter, start, testRatings);
                            ...
                            //    hr_vec[iter] = hits.mean();
                            //    ndcg_vec[iter] = ndcgs.mean();
                             //   precs_vec[iter] = precs.mean();
                             //   if(iter>=1)
                              //      if( Math.abs(hr_vec[iter]-hr_vec[iter-1])< 0.0005)
                               //     {   
                                //        System.out.printf("total Iter=%d\n", iter);
                                 //       return;
                                 //   }
