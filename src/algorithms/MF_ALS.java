...
public class MF_ALS extends TopKRecommender {
  ...
  /** Caches */
	public DenseMatrix SU;
	public DenseMatrix SV;
	...
	public void buildModel() throws IOException{
		System.out.println("Run for MF_ALS");
		 PrintWriter writer = new PrintWriter (new FileOutputStream("MF_ALS.progress"));
	...
	private void update_user(int u) {
		...
		// Get matrix Au   
		DenseMatrix Au = SV.scale(w0);     //CORRECT "SU" to "SV"
	...
	private void update_item(int i) {
		...
		// Get matrix Ai  
		DenseMatrix Ai = SU.scale(w0);  //CORRECT "SV" to "SU"
		for (int k1 = 0; k1 < factors; k1 ++) {
			for (int k2 = 0; k2 < factors; k2 ++) {
				for (int u : userList)
